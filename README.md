...
1. **确认目标**：找对页面，确认 Figma 节点、目标路由、当前分支和可以修改的范围。
2. **检查工具**：确认所需 Skill、Figma MCP 和浏览器工具真的能用。安装了不等于已经配置好。
3. **读取设计**：查看同一个 Figma 节点的结构和截图，弄清布局、状态、素材和响应式要求。
4. **查看项目**：寻找已有组件、路由、接口、多语言和测试写法，能复用就不重做。
5. **确认业务**：用 PRD、接口和现有代码核对交互。不清楚的部分不替业务做决定。
6. **分块开发**：先做已经确认的内容，每次改动尽量小，方便检查和回退。
7. **真实验收**：打开实际路由，操作页面，检查请求、测试和代码改动；没有证据就不说完成。

```mermaid
flowchart TD
  A["开始：拿到 Figma 链接和目标页面"] --> B["1. 找对页面"]
  B --> C{"2. 工具能正常读取 Figma 吗？"}
  C -->|不能| D["补 Skill、MCP 或授权"]
  D --> E{"现在能读取精确节点吗？"}
  E -->|不能| X["视觉部分先停"]
  X --> Y["其他有依据的修改可以继续"]
  E -->|能| F["3. 看懂设计"]
